...
var _conditional = require("../utils/conditional");
const {
  add,
  multiply
...
function forFadeFromBottomAndroid(_ref4) {
  ...
  const opacity = (0, _conditional.conditional)(closing, current.progress, current.progress.interpolate({
...
function forScaleFromCenterAndroid(_ref6) {
  ...
  const scale = (0, _conditional.conditional)(closing, current.progress.interpolate({
    inputRange: [0, 1],
    outputRange: [0.925, 1],
    extrapolate: 'clamp'
